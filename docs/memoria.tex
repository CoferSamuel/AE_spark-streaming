...
\item
  \protect\hyperlink{retos-y-soluciones-en-la-infraestructura}{Retos y Soluciones}
\item
  \protect\hyperlink{conclusiones}{Conclusiones}
\item
  \protect\hyperlink{trabajo-futuro}{Trabajo Futuro}
\item
  \protect\hyperlink{bibliografuxeda-y-referencias}{Bibliografía y Referencias}
\end{enumerate}
...
proyecto un enfoque más diferente, novedoso y moderno. El objetivo fue implementar
...
con todas sus dependencias} en contenedores aislados. Docker puede considerarse
como un contenedor hermético de transporte. Este contenedor incluye:
\begin{itemize}
...
\textbf{Ventaja:} El contenedor funciona de manera idéntica en cualquier entorno de desarrollo
o en un servidor de producción. Independientemente del sistema operativo anfitrión (Windows, Mac o
Linux), Docker gestiona la compatibilidad.
...
\tightlist
\item
  \textbf{Reproducibilidad:} Todo el equipo dispone del mismo
  entorno exacto. Se evitan discrepancias de configuración entre entornos locales.
\item
  \textbf{Aislamiento:} Kafka y Zookeeper corren en sus propios
  contenedores sin interferir con el sistema operativo anfitrión.
\item
  \textbf{Facilidad:} En lugar de instalar Java, descargar Kafka,
  configurar todo manualmente (un proceso manual complejo y propenso a errores), ejecutamos un
  comando: \texttt{docker\ compose\ up\ -d}. ¡Listo en cuestión de segundos!
\item
  \textbf{Escalabilidad:} Si fuera necesario utilizar 5 brokers de Kafka en lugar
  de 1, solo se cambiaría el archivo de configuración. Sin Docker, sería
...
La arquitectura se organiza de la siguiente manera: En el ordenador anfitrión
...
Sin embargo, para que las aplicaciones Python (que corren en la máquina
...
escucha en el puerto \textbf{2181} (desde la perspectiva del host, es
\texttt{localhost:2181}) y Kafka en el puerto \textbf{9092}
(\texttt{localhost:9092}).

De esta forma, el código Python no necesita saber que estas aplicaciones
están dentro de contenedores; simplemente se conecta a
\texttt{localhost:9092} como si estuvieran instaladas directamente en la
...
Kafka puede compararse con una gran oficina de correos. \textbf{Zookeeper}
...
\item \textbf{Topic:} Es como una ``carpeta'' o ``canal'' dentro de Kafka. Se ha creado uno llamado \texttt{tweets\_topic} donde se vuelcan todos los mensajes simulados.
\end{itemize}
...
En lugar de instalar Java, Kafka y Zookeeper manualmente en el entorno
de desarrollo (lo cual suele dar errores de versiones), creamos un
archivo \texttt{docker-compose.yml}. Este archivo es una ``receta'' que
indica a Docker cómo levantar todo el entorno con un solo comando.
...
\item \textbf{\texttt{KAFKA\_ADVERTISED\_LISTENERS}}: Este fue el mayor reto. Por defecto, Kafka anuncia su IP interna de Docker. Fue necesario configurarlo para que anuncie \texttt{localhost:9092}, permitiendo así que los scripts de Python (que corren en el host, fuera de Docker)
\end{itemize}
puedan conectarse.
...
está arriba, hay que crear el canal donde viajarán los datos. Se utiliza
...
\textbf{Paso 4: Detener / Apagar la infraestructura} Al finalizar el
desarrollo o para limpiar el entorno, es importante detener los
...
Los contenedores se pausan pero siguen existiendo. Se pueden reanudar
...
\textbf{Nota importante:} Si se utilizan volúmenes persistentes (datos
almacenados en disco), se debe agregar el flag para eliminarlos también:
...
La función del \textbf{Responsable de la Ingesta} fue crear la
...
\item
  ...
  de tasa (\emph{rate-limits}) y costes asociados. Un simulador
...
\item
  ...
  Streaming (agrupaciones y ventanas de tiempo), era necesario asegurar
  que ciertos hashtags (ej: \texttt{\#Spark}) aparecieran con más
  frecuencia que otros. Un generador sintético permite ``ajustar''
  las probabilidades para hacer la demo más visual.
\item
  \textbf{Velocidad Ajustable:} Es posible acelerar o frenar el flujo de
...
puros, por lo que el productor se encarga de transformar los
...
Para facilitar el trabajo del \textbf{Consumidor (Persona C)}, se diseñó un
esquema de datos JSON limpio pero enriquecido. En lugar de enviar solo
texto plano, se envía una estructura estructurada:
...
\item
  ...
  mucho con \#Spark'').
\item
  \textbf{\texttt{hashtag\_principal}}: Se aplicó una lógica de
  pre-procesamiento. Aunque el hashtag está en el texto, se envía
...
último minuto'', recalculado y reimpreso de forma periódica cada 10
segundos.
...
\newpage
\hypertarget{retos-y-soluciones-en-la-infraestructura}{%
\subsection{Retos y Soluciones}\label{retos-y-soluciones-en-la-infraestructura}}

Durante la implementación de la infraestructura, el equipo se enfrentó a desafíos técnicos significativos que requirieron investigación y adaptación.

\textbf{Compatibilidad de Imágenes Docker (Bitnami vs Confluent)}

Uno de los mayores desafíos fue la compatibilidad de las imágenes de Docker. Inicialmente se intentó utilizar imágenes de Bitnami, pero surgieron conflictos con la arquitectura del procesador y las versiones de las librerías cliente.

\textbf{Solución:} Se optó por migrar a las imágenes oficiales de \textbf{Confluent Inc.} (basadas en la plataforma Enterprise de Kafka). Además, fue necesario fijar versiones específicas (\emph{pinning versions}) en el archivo \texttt{docker-compose.yml} (versión \texttt{7.4.4}) en lugar de usar la etiqueta \texttt{latest}, garantizando así que todos los miembros del equipo trabajaran sobre la misma base binaria y evitando errores de "versión no encontrada" al desplegar en diferentes máquinas.

\textbf{Gestión de Recursos y "OOM Kills"}

Otro obstáculo fue el consumo de memoria en los ordenadores portátiles del equipo. Al levantar el stack completo, ocasionalmente el contenedor de Kafka se detenía abruptamente con el código de salida 137 (Out of Memory).

\textbf{Solución:} Se optimizó la configuración de la Java Virtual Machine (JVM) dentro del contenedor. Mediante la variable de entorno \texttt{KAFKA\_HEAP\_OPTS}, se limitó la asignación de memoria a valores más conservadores (ej. "-Xmx512M -Xms512M"), logrando un equilibrio entre rendimiento y estabilidad para un entorno de desarrollo local.
...
En resumen, se ha logrado construir un pipeline completo de datos
...
Si se dispusiera de más tiempo para la evolución del proyecto, una mejora clave sería la integración de un sistema de visualización avanzado. Actualmente, los resultados se muestran por consola, pero sería ideal conectar la salida de Spark a un dashboard visual como \textbf{Grafana} o \textbf{Kibana}. Esto permitiría observar gráficas de tendencias en tiempo real, facilitando la toma de decisiones basada en datos de una manera mucho más intuitiva y profesional.

\newpage
\hypertarget{bibliografuxeda-y-referencias}{%
\subsection{Bibliografía y Referencias}\label{bibliografuxeda-y-referencias}}

Para el desarrollo de este proyecto, se ha consultado la documentación oficial de las tecnologías utilizadas, garantizando el uso de buenas prácticas y patrones de diseño estándar en la industria.

\begin{itemize}
\item
  \textbf{Apache Kafka Documentation}: \url{https://kafka.apache.org/documentation/}
\item
  \textbf{Spark Structured Streaming Programming Guide}: \url{https://spark.apache.org/docs/latest/structured-streaming-programming-guide.html}
\item
  \textbf{Docker Hub - Confluent Inc Images}: \url{https://hub.docker.com/u/confluentinc}
\item
  \textbf{Kafka-Python Library}: \url{https://kafka-python.readthedocs.io/en/master/}
\end{itemize}
